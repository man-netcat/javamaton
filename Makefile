...
MAIN_FILE_CLASS = $(MAIN_FILE:src/%.java=bin/%.class)


# Get all .java files
JAVA_FILES = $(shell find src -iname '*.java' | grep -v '^${MAIN_FILE}$$')
...
$(MAIN_FILE_CLASS): bin/%.class: src/%.java
	javac -sourcepath bin/ -cp bin/ -d bin/ -s bin/ -h bin/ $<

# Build total package
main: $(JAVA_FILES_CLASS) $(MAIN_FILE_CLASS)
